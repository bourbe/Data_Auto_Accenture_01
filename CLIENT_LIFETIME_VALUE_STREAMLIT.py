...
st.write(""" ## 1. Snapshot of the data""")
st.write('Below is a snapshot is listed of the first 5 records from the created dataframe')
st.write('---')

st.write(df_shapley.head(5))


st.write(""" ## 2. Exploration of the categorical data""")
st.write('Select a X_CAT categorical data and cross it with the Client Lifetime Value, the number of polices or the nuber of monts since inception')
st.write('---')
...
y_cat =  st.selectbox('Y_CAT',['Clv','Months.Since.Policy.Inception','Number.of.Policies'])


pivot = pd.pivot_table(df_shapley,index=[x_cat],values=[y_cat],aggfunc= np.mean)
pivot[y_cat] = pivot.mean(axis=1).round(0)
...
st.write(""" ## 3. Exploration of the numerical data""")
st.write('Select a X_NUM numerical data and cross it with the Client Lifetime Value or the number of polices')
st.write('---')
...
y_num =  st.selectbox('Y_NUM',['Clv','Number.of.Policies'])
...
st.write('This is the global interpretation of model : The most importants variables for the Client Lifetime Value prediction')
...
## Linear Regression
st.write('## 4.Predicted Value Of The Client Value Lifetime - Linear Regression')
st.write('---')
